...
class DenseGGNNChemModel(ChemModel):
    ...
    def generate_graph_with_state(self, random_normal_states, random_normal_states_in, num_vertices,
                                  generated_all_smiles, elements, count, cnn_input):
        ...
        node_symbol_batch_feed_dict=self.get_dynamic_feed_dict(elements, None, None,
                                     ...
                                     random_normal_states, random_normal_states_in, 0, cnn_input)
        ...
        for idx in starting_point: 
            ...
            new_mol, total_log_prob=self.search_and_generate_molecule(idx, np.copy(valences),
                                                ...
                                                random_normal_states, random_normal_states_in, elements, num_vertices, cnn_input)
    ...
    def generate_new_graphs(self, data, struct_data):
        # Prepare data
        ...
        for step in range(len(bucket_at_step)):
            ...
            for d in elements:
                elements_order.append(d['sdf_idx'])
        structural_data = [struct_data[i] for i in elements_order]
        # Reset bucket_counters
        bucket_counters = defaultdict(int)
        # Write new types file for this epoch
        with open('generation_epoch_types', 'w') as f:
            for d in structural_data:
                f.write(d+'\n')
        # Setup libmolgrid (structural info)
        e = molgrid.ExampleProvider(data_root=self.params['struct_data_root'], balanced=False, shuffle=False)
        e.populate('generation_epoch_types')
        gmaker = molgrid.GridMaker()
        dims = gmaker.grid_dimensions(e.num_types())
        tensor_shape = (self.params['batch_size'],)+dims
        labels_grid = molgrid.MGrid1f(self.params['batch_size'])
        ...
        for step in range(len(bucket_at_step)):
            ...
            # Load structural data for CNN
            batch = e.next_batch(self.params['batch_size'])
            gmaker.forward(batch, input_tensor, random_translation=0, random_rotation=False)
            batch.extract_label(0, labels_grid)
            # Iterate over elements in batch
            for i, elements in enumerate(elements_batch):
                ...
                # Get CNN input
                if self.params['cnn_ligand_only']:
                    cnn_input = np.array([input_tensor.tonumpy()[i, 14:, :, :, :]])
                else:
                    cnn_input = np.array([input_tensor.tonumpy()[i]])
                ...
                for _ in range(self.params['number_of_generation_per_valid']):
                    ...
                    self.generate_graph_with_state(random_normal_states, random_normal_states_in,
                                       maximum_length, generated_all_smiles, elements, count, cnn_input)
                    count+=1
            bucket_counters[bucket] += 1
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                ...
                self.placeholders['sdf_idx']: batch_data['sdf_idx'],
